...
#undef _CONVERT_
  ...
  nvhttp_t nvhttp {
    ...
    { "60", "90", "120", "144" },  // supported fps (supports fractional refresh rates)
  ...
  sunshine_t sunshine {
    "en",  // locale
    "en",  // tray_locale (tray menu language)
    2,  // min_log_level
    0,  // flags
    {},  // User file
    {},  // Username
    ...
    false,  // restore_log - by default, do not restore the log file
    50,  // max_log_size_mb - default 50MB; rotates automatically when exceeded
  ...
  void apply_config(std::unordered_map<std::string, std::string> &&vars) {
    ...
    // System tray menu language setting
  ...
  update_config(const std::map<std::string, std::string> &updates) {
    try {
      ...
      // Update config entries while tracking whether anything changed
      bool hasChanged = false;
      for (const auto &[key, value] : updates) {
        auto it = configMap.find(key);
        if (value.empty() || value == "null") {
          // Empty value: remove this config entry
          if (it != configMap.end()) {
            configMap.erase(it);
            hasChanged = true;
          }
      ...
      // Write the configuration file in alphabetical order
  ...
  update_full_config(const std::map<std::string, std::string> &fullConfig) {
    try {
      // Read-only fields that should not be saved to the config file (API response fields, not config entries)
      const std::set<std::string> readonlyFields = {
        "status",           // API response status, not a config entry
        "platform",         // Platform info, determined at compile time, read-only
        "version",          // Version number, read-only
        "display_devices",  // Display device list, enumerated at runtime, read-only
        "adapters",         // Adapter list, enumerated at runtime, read-only
        "pair_name",        // Pairing name, generated by the system, read-only
      };

      // Protected fields: controlled by the system tray; preserve the existing value from the local config file
      const std::set<std::string> protectedFields = {
        "vdd_keep_enabled",       // Controlled by the system tray, not modifiable via the web UI
        "vdd_headless_create",    // Controlled by the system tray, not modifiable via the web UI
        "tray_locale",            // Controlled by the system tray, not modifiable via the web UI
      };

      // Read the existing configuration file (used to fetch protected-field values and for later comparison)
      ...
      // Use std::map to ensure alphabetical save order
      std::map<std::string, std::string> resultMap;

      // Collect incoming config entries (skipping read-only and protected fields)
      for (const auto &[key, value] : fullConfig) {
        // Skip read-only fields
        if (readonlyFields.count(key)) {
          continue;
        }
        // Skip protected fields (overwritten later with local values)
        if (protectedFields.count(key)) {
          continue;
        }
        // Skip empty values and null
        if (value.empty() || value == "null") {
          continue;
        }
        resultMap[key] = value;
      ...
      // Add protected fields (using existing values from the local config file)
      ...
      // Compare new config with the original; skip writing if identical
