...
namespace nvenc {
  ...
  nvenc_base::create_encoder(
    ...
    // Save the current encoder config and init params for later dynamic adjustments
    saved_init_params = init_params;
    current_enc_config = enc_config;
    saved_init_params.encodeConfig = &current_enc_config;  // Ensure the pointer references our member variable
  ...
  nvenc_base::set_bitrate(int bitrate_kbps) {
    ...
    if (NVENC_INT_VERSION < 1100) {
      BOOST_LOG(error) << "NvEnc: NVENC API version too old (" << NVENC_INT_VERSION << "), dynamic bitrate adjustment not supported";
      return;
    }
    ...
    // Copy the current config and prepare to modify it
    ...
    // HEVC needs the VBV buffer adjusted to handle the larger bitrate
    if (is_hevc) {
      ...
      // Prevent the VBV buffer from being too small
      if (new_vbv_size < 1000 * 100) new_vbv_size = 1000 * 100;  // At least 100K
      enc_config.rcParams.vbvBufferSize = new_vbv_size;
      BOOST_LOG(debug) << "NvEnc: VBV buffer adjusted to " << new_vbv_size / 1000 << " Kbps";
    }
    ...
    // When raising the HEVC bitrate, reset the encoder state
